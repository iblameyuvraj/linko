...
export default function Navbar() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Set Supabase client session so we can use supabase.auth on client
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/signup", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ username: signupUsername.trim(), password: signupPassword }),
            });
            const json = await res.json();
            if (!res.ok || json?.error) {
    ...
    return (
        <>
            ...
            {/* Auth Modals */}
            <Modal isOpen={loginOpen} onClose={() => setLoginOpen(false)} title="Log in to your account">
            ...
            <Modal isOpen={signupOpen} onClose={() => setSignupOpen(false)} title="Create your account">
